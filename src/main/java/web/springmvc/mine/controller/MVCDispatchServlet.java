...
import web.springmvc.mine.annotation.*;
...
public class MVCDispatchServlet extends HttpServlet {
    ...
    private void parseProperties(ServletConfig config) {
        try {
            ...
            InputStream inputStream = getClass().getClassLoader()
                    .getResourceAsStream(packagePath.replace("classpath:", ""));
    ...
    private void scanClassFile(String packagePath) {
        String filePath =
                getClass().getClassLoader().getResource("").getPath() + packagePath.replaceAll("\\.", "/");
                            ...
                            for (Annotation annotation2 : fieldAnnotations) {
                                if (annotation2 instanceof MVCAutowired) {
                                    ...
                                    if (object == null) {
                                        ...
                                        field.set(objectMap.get(clazz.getName()),
                                                objectMap.get(field.getType().getName()));
                                    }
                                }
                            }
                        }
